...
export const ParameterUpdateDescription = ({ selectedParam }: { selectedParam: ConfigParam | undefined }) => {
  ...
  return (
    <Flex flexDirection='column' gap={4} paddingTop={4}>
      ...
      {isListParam && (
        <Fragment>
          <Text textStyle='small'>A list of values common for all the selected configurations is displayed.</Text>
          <Text textStyle='small'>
            "*" indicates that at least one configuration has values other than the common ones.
            <ExpandableText>
              <Text textStyle='small'>The list can be modified as follows:</Text>
              <List ordered={false}>
                <Text textStyle='small'>
                  enter one or more new values to add them to all the selected configurations
                </Text>
                <Text textStyle='small'>
                  delete one or more of the common values to remove them from all the selected configurations
                </Text>
                <Text textStyle='small'>
                  delete "*" (if present) to remove from all the selected configurations any values different than the
                  ones displayed here.
                </Text>
              </List>
            </ExpandableText>
