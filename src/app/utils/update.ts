...
} from '@dynatrace-sdk/client-classic-environment-v1/types/packages/client/classic-environment-v1/src/lib/models/browser-synthetic-monitor-update';
...
const createUpdateDto: (config: SyntheticMonitor) => SyntheticMonitorUpdate =
  (config) => {
    const updateDto: SyntheticMonitorUpdate = {
      ...
      tags: [...config.tags],
      type: config.type === SyntheticMonitorType.Http ? SyntheticMonitorUpdateType.Http : SyntheticMonitorUpdateType.Browser,
    };
    if (config.type === SyntheticMonitorType.Browser) {
      ...
      (updateDto as BrowserSyntheticMonitorUpdate).keyPerformanceMetrics = browserConfig.keyPerformanceMetrics ?
        { ...browserConfig.keyPerformanceMetrics } : undefined;
    }

    return updateDto;
  };

const getUpdatedList: (existing: Array<string>, initial: Array<string>, changed: Array<string> | undefined) => Array<string> =
  (existing, initial, changed) => {
    if (!changed) {
      return existing;
    }
...
const tagToTagWithSource: (tag: TagParam) => TagWithSourceInfo =
  (tag) => {
    return {
...
const tagToString: (tag: TagParam | Wildcard) => string =
  (tag) => {
...
const getUpdatedTags: (
  ...
  (existing, initial, changed) => {
    ...
    return existingAutoTags.concat(updatedList.map(tag => {
      const [key, value] = tag.split(keyValueSeparator);
      return tagToTagWithSource({ key, value: value.length > 0 ? value : undefined });
    }));
  };
...
export const getUpdateDto: (
  ...
  (existing, initial, changed, mode) => {
    ...
    if (mode === DisplayMode.SAME_TYPE) {
      updateDto.frequencyMin = typeof changed.frequencyMin === 'number' ? changed.frequencyMin : updateDto.frequencyMin;
      updateDto.manuallyAssignedApps =
        getUpdatedList(existing.manuallyAssignedApps, initial.manuallyAssignedApps, changed.manuallyAssignedApps);
    }
...
export const getUpdateDtoForCurrentSelection: (
  ...
  (existing, initial, changed, selectedParam) => {
    const updateDto = createUpdateDto(existing);
    if (!selectedParam) {
      return updateDto;
    }
